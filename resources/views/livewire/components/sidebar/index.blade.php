<div x-data='initSidebar'>
    ...
    <div class="fixed inset-0 z-40 transition-opacity duration-200 bg-primary-yellow bg-opacity-30 lg:hidden lg:z-auto"
        :class="sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'" aria-hidden="true" x-cloak></div>

    <!-- Sidebar -->
    <div id="sidebar"
        class="flex bg-gradient-to-r to-primary-blue from-primary-blue-light flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 shrink-0  p-4 transition-all duration-200 ease-in-out"
        ...
        <div class="flex justify-between pr-3 mb-10 sm:px-2">
            ...
            <a class="block" href="{{ route('index') }}">
                <h1 class="text-2xl font-bold tracking-wide text-center text-primary-yellow-light">DR. PAWS</h1>
                <p class="text-sm font-light leading-4 tracking-wide text-center text-white ">Veterenary Clinic and
                    Grooming Center</p>
            </a>
        </div>
                ...
                <ul class="mt-3">
                    ...
                    <li class="px-3 py-2 rounded-sm mb-0.5 last:mb-0">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div
                                    ...
                                    <svg class="w-3 h-3 ml-1 text-white fill-current shrink-0"
                                        :class="showMenu('{{$sidebar['unique_name']}}') ? 'rotate-180 transition-transform ease-linear' : 'rotate-0 transition-transform ease-linear'"
                                        viewBox="0 0 12 12">
                        ...
                        <div class="lg:hidden lg:sidebar-expanded:block 2xl:block">
                            <ul class="mt-1 pl-9" x-show="showMenu('{{$sidebar['unique_name']}}')" x-transition>
    ...
    <script>
        document.addEventListener('livewire:init', () =>{
            Alpine.data('initSidebar', function(){
                return {
                    recordMenu: false,
                    serviceOfferedMenu: false,
                    authenticationMenu: false,
                    pointOfSalesMenu:false,
                    toggleRecordMenu(key){
                        switch (key) {
                            case 'recordManagement':
                                this.sidebarExpanded ? this.recordMenu = ! this.recordMenu : this.sidebarExpanded = true
                                break;
                            case 'serviceOffered':
                                this.sidebarExpanded ? this.serviceOfferedMenu = ! this.serviceOfferedMenu : this.sidebarExpanded = true
                                break;
                            case 'pointOfSales':
                                this.sidebarExpanded ? this.pointOfSalesMenu = ! this.pointOfSalesMenu : this.sidebarExpanded = true
                                break;
                            default:
                                break;
                        }
                    ...
                    showMenu(key){
                        switch (key) {
                            case 'recordManagement':
                                ...
                                break;
                            case 'serviceOffered':
                                return this.serviceOfferedMenu
                                break;
                            case 'pointOfSales':
                                return this.pointOfSalesMenu
                                break;
                            default:
                                break;
                        }
